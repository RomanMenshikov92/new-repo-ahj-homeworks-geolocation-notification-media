...
export default class TimelineControl {
  ...
  */
  async onPopUpSubmit(value) {
    ...
    /* если нажата кнопка отмена, закрываем модальное окно */
    if (dataID === 'popup-cancel') { this.timelineDOM.popupClose(); }
    ...
    if (dataID === 'popup-submit') {
      ...
      *  если получилось, то разрешаем отправку сообщения
      */
      if (geoStr === '') {
      ...
      const geoObj = geoValidation(geoStr); // валидация введённых значений
  ...
  /*
  *  кнопка ОК на панели медиа, обрабатывает записанное медиа в пост
  */
  async onMediaOk() {
    clearInterval(this.secondMediaID); // остановка счётчика времени на панели инфо
    ...
    const { url, type } = await this.recorder.getRecordingAsBlob(); // получаем blob ссылку и тип
  ...
  */
  onMediaCancel() {
    ...
    this.timelineDOM.mediaCloseMedia(); // закрытие медиа панели
    this.timelineDOM.removeStreamVideo(); // удаление предпросмотр видео, если оно было
  }
